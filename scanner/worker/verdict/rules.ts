...
import type { LinkChain, RuleFinding, Verdict } from '../../shared/contract'
import { escalate } from './verdict'

/**
 * The slice of `ScannerConfig` these rules depend on. Declaring exactly the
 * fields consumed (rather than the whole config) keeps the rule engine decoupled
 * from `worker/config.ts` and trivially testable: a caller supplies just the
 * shortener allowlist.
 */
