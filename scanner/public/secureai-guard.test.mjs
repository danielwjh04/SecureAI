...
import { browserGuardHealth, runGuard } from './secureai-guard.mjs'
...
test('reports hook health without exposing secrets', () => {
  const health = browserGuardHealth({
    ...
    SECUREAI_INTEGRATION_VERSION: 'browser-test',
  })

  assert.equal(health.provider, 'browser')
