...
When `GUARD_TICKET_SECRET` is set, or an ES256 JWK pair is configured with `GUARD_TICKET_PRIVATE_JWK` and `GUARD_TICKET_PUBLIC_JWK`, `/api/guard` also returns a short-lived signed allow ticket for an exact repeated action. Tickets include `alg` and `kid`, and bind to the action hash, policy version, trust revision, project scope, device identity, and integration version. `SCANNER_GUARD_TICKET_TTL_S` controls expiry, and a changed action or revision forces a fresh decision.
